...
module$PreziPage$default.PreziPage = class extends module$node_modules$react$default.Component {
  constructor() {
    ...
    selectedTemplate:this.props.preziOptions.selectedTemplate || null, useSimplifiedTemplateChooser:this.props.preziOptions.simplifiedTemplateChooser || false, skipToPrivacyForm:false, ppwUpgradeToEduPlusEnabled:this.props.preziOptions.ppwUpgradeToEduPlusEnabled || false, afterCreateAction:null, privacyPaywallEnabled:this.props.preziOptions.privacyPaywallEnabled || false, shouldShowPrivacyForm:this.props.preziOptions.privacyPaywallEnabled || false, segmentationAnswer:null, defaultTemplateCategory:this.props.preziOptions.selectedTemplateCategory, 
    templatePaywallEnabled:false};
        ...
        switch(event.data.choice) {
          case "blank":
            if (this.state.privacyPaywallEnabled) {
              ...
              this.startEditingWithTemplateId("blank_-_white:blank_-_white_default", "BLANK_TEMPLATE_CATEGORY", false, false, "storyblocks");
            }
            break;
          case "brand":
            this.startEditingWithTemplateId("branding_v1", "UNDEFINED_TEMPLATE_CATEGORY", false, false, "branding");
            break;
          case "pptx":
            this.startEditingWithTemplateId("ppt_20_min_w48", "IMPORT_PPT_TEMPLATE_CATEGORY", true, false, null);
            break;
          case "skip":
  ...
  componentWillMount() {
    ...
    this.startEditingControllerPromise = new Promise((resolve) => {
      this.setStartEditingController = (controller) => {
        const startEditingController = {startEditing:(createOptions, onError, action) => {
          controller.startEditing(createOptions, onError, action);
        }};
  ...
  startEditingWithTemplateId(templateId, templateCategory, pptWorkflow, preziVideoTemplate, afterCreateAction) {
    this.startEditingControllerPromise.then((startEditingController) => {
      startEditingController.startEditing({templateId, templateCategory, pptTemplateFromSegmentation:pptWorkflow, preziVideoTemplate}, () => {
      }, afterCreateAction);
    });
  ...
  handleProgressEvent(event) {
    ...
    if (event.eventType === 5) {
      preziPlayerJS.getEditor().getEditorStore().getUserLicenseInformationStream().onValue((uli) => {
        this.setState({templatePaywallEnabled:!uli.featureSet.pitchPrivacyPrivate && this.featureSwitcher.isActive("js-gm-paywall-present-templates")});
        const privacyPaywallEnabled = !uli.featureSet.pitchPrivacyPrivate && !this.featureSwitcher.isActive("gm-cb-disable-privacy-paywall");
        ...
        if (this.state.pageMode === 3 && this.state.shouldShowSegmentation) {
          ...
          if (this.state.pageMode === 3 && !this.state.waitingForSegmentationResponse && this.state.templateListLoadingStatus === "loaded" && this.state.templateChooserUI != null && this.state.templateService != null) {
            if (this.state.isEditorReady) {
              ...
              this.state.useSimplifiedTemplateChooser), defaultTemplateCategory:this.state.defaultTemplateCategory, templatePaywallEnabled:this.state.templatePaywallEnabled});
            }
          } else {
            let infoModeContent;
            let miniModeContent;
